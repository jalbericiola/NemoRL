...
strict_pair_select_bootstrap_sha256sum() {
  ...
  if [[ -n "${STRICT_PAIR_BOOTSTRAP_SHA256SUM:-}" ]]; then
    case "${STRICT_PAIR_BOOTSTRAP_SHA256SUM}" in
      /usr/bin/sha256sum|/sbin/sha256sum) ;;
      *)
        strict_pair_error "bootstrap sha256sum must be one fixed system path."
        return
        ;;
    esac
  elif [[ -f /usr/bin/sha256sum && ! -L /usr/bin/sha256sum && \
          -x /usr/bin/sha256sum ]]; then
    STRICT_PAIR_BOOTSTRAP_SHA256SUM=/usr/bin/sha256sum
  elif [[ -f /sbin/sha256sum && ! -L /sbin/sha256sum && \
          -x /sbin/sha256sum ]]; then
    STRICT_PAIR_BOOTSTRAP_SHA256SUM=/sbin/sha256sum
  else
    strict_pair_error "no supported fixed bootstrap sha256sum is available."
    return
  fi
...
# The manifest hash and bootstrap digest are supplied by the sealed
# deployment package; the manifest itself is also checked against the NeMo-RL
# runnable manifest later in strict_pair_verify_deployment.
strict_pair_load_runtime_tools() {
  ...
  : "${STRICT_PAIR_RUNTIME_TOOL_MANIFEST:?STRICT_PAIR_RUNTIME_TOOL_MANIFEST is required}"
  : "${STRICT_PAIR_HOST_PYTHON:?STRICT_PAIR_HOST_PYTHON is required}"
  : "${STRICT_PAIR_CONTAINER_PYTHON:?STRICT_PAIR_CONTAINER_PYTHON is required}"
  : "${STRICT_PAIR_CONTAINER_UV:?STRICT_PAIR_CONTAINER_UV is required}"
  : "${STRICT_PAIR_UV_SHIM:?STRICT_PAIR_UV_SHIM is required}"
  for name in \
    ...
    EXPECTED_STRICT_PAIR_UV_SHIM_SHA256; do
    strict_pair_require_lower_digest_value "${name}"
  done

  strict_pair_select_bootstrap_sha256sum
  case "${STRICT_PAIR_HOST_PYTHON}" in
    /cm/local/apps/python312/bin/python3.12) ;;
    /usr/bin/python3)
      if [[ "${OSTYPE}" != darwin* ]]; then
        strict_pair_error "/usr/bin/python3 is allowed only by the macOS hermetic-test contract."
        return
      fi
      ;;
    *)
      strict_pair_error "host Python must be one code-fixed root-owned system path."
      return
      ;;
  esac
  ...
  records="$(
    "${STRICT_PAIR_HOST_PYTHON}" -I -B - \
      ...
      "${EXPECTED_STRICT_PAIR_CONTAINER_UV_SHA256}" \
      "${STRICT_PAIR_UV_SHIM}" \
      "${EXPECTED_STRICT_PAIR_UV_SHIM_SHA256}" <<'PY'
import hashlib
import json
import os
import pathlib
...
expected_pairs = {
    ...
    ("container", "python"): (container_python, container_python_sha256),
    ("container", "uv"): (container_uv, container_uv_sha256),
    ("container", "uv_shim"): (uv_shim, uv_shim_sha256),
}
...
PY
  ...
  while IFS=$'\t' read -r scope name path digest; do
    ...
    case "${name}" in
      awk) STRICT_PAIR_TOOL_AWK="${path}" ;;
      bash) STRICT_PAIR_TOOL_BASH="${path}" ;;
      cat) STRICT_PAIR_TOOL_CAT="${path}" ;;
      chmod) STRICT_PAIR_TOOL_CHMOD="${path}" ;;
      cmp) STRICT_PAIR_TOOL_CMP="${path}" ;;
      date) STRICT_PAIR_TOOL_DATE="${path}" ;;
      env)
        STRICT_PAIR_TOOL_ENV="${path}"
        STRICT_PAIR_TOOL_ENV_SHA256="${digest}"
        ;;
      find) STRICT_PAIR_TOOL_FIND="${path}" ;;
      git) STRICT_PAIR_TOOL_GIT="${path}" ;;
      grep) STRICT_PAIR_TOOL_GREP="${path}" ;;
      ln) STRICT_PAIR_TOOL_LN="${path}" ;;
      mkdir) STRICT_PAIR_TOOL_MKDIR="${path}" ;;
      mktemp) STRICT_PAIR_TOOL_MKTEMP="${path}" ;;
      nvidia_smi) STRICT_PAIR_TOOL_NVIDIA_SMI="${path}" ;;
      python) STRICT_PAIR_TOOL_PYTHON="${path}" ;;
      readlink) STRICT_PAIR_TOOL_READLINK="${path}" ;;
      realpath) STRICT_PAIR_TOOL_REALPATH="${path}" ;;
      rm) STRICT_PAIR_TOOL_RM="${path}" ;;
      rsync) STRICT_PAIR_TOOL_RSYNC="${path}" ;;
      sbatch)
      ...
      scontrol)
        ...
        ;;
      sha256sum) STRICT_PAIR_TOOL_SHA256SUM="${path}" ;;
      stat) STRICT_PAIR_TOOL_STAT="${path}" ;;
      wc) STRICT_PAIR_TOOL_WC="${path}" ;;
      *)
        strict_pair_error "unexpected runtime host tool after validation: ${name}"
        return
        ;;
    esac
  done <<< "${records}"
...
PY
  ...
  for source_rel in \
    ...
    examples/nemo_gym/nemotron-3.5-nano/single_env_freeform_sc.yaml; do
...
def recovery(records):
    return {
        "argv": [scheduler_tools["scontrol"]["path"], "show", "job", "--oneliner"],
        "byte_count": 1,
...
document = {
    ...
    "receipt": {
        ...
        "result_marker": {
            ...
            "grammar": (
                ...
                "nonce=<nonce> "
                "off_(job_id|candidate_job_id)=<decimal-or-none> "
                "off_identity_authenticated=<true|false> "
                "on_(job_id|candidate_job_id)=<decimal-or-none> "
    ...
    "scheduler": {
        ...
        "recovery_query": {
            ...
            "require_every_line_parseable": True,
            "require_exactly_one_identity_match": True,
