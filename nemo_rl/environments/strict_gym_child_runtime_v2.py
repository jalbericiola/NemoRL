...
STRICT_GYM_FORMAT_CALL_SCHEMA = "nemo-rl-strict-format-verification-call-v1"
STRICT_GYM_FORMAT_CLOSED_SCHEMA = "nemo-rl-strict-format-verification-closed-v1"
STRICT_GYM_FORMAT_CALL_INDEX_SCHEMA = "nemo-rl-strict-format-verification-call-index-v1"
...
def _require_sealed_bootstrap_root() -> tuple[Path, str]:
    root = Path(__file__).resolve(strict=True).parent / "_strict_gym_child_bootstrap_v2"
    ...
    if value_type is dict:
        for key, item in value.items():
            if type(key) is not str:
                raise TypeError(f"{name} key is not an exact string")
            _require_exact_format_json(item, name=name)
    elif value_type is list:
        for item in value:
            _require_exact_format_json(item, name=name)
    elif value_type not in (str, int, float, bool, type(None)):
        raise TypeError(f"{name} contains a non-exact JSON value")


def _format_response_text(response: Mapping[str, Any]) -> str:
    if type(response) is not dict:
        raise TypeError("format verifier response must be an exact dict")
    output = response.get("output")
    if type(output) is not list:
        raise TypeError("format verifier response output must be a list")
    pieces: list[str] = []
    for item in output:
        if type(item) is not dict:
            raise TypeError("format verifier response item must be an object")
        if item.get("type") != "message":
            continue
        content = item.get("content")
        if type(content) is not list:
            raise TypeError("format verifier message content must be a list")
        for part in content:
            if type(part) is not dict:
                raise TypeError("format verifier content item must be an object")
            if part.get("type") == "output_text":
                text = part.get("text")
                if type(text) is not str:
                    raise TypeError("format verifier output text must be a string")
                pieces.append(text)
    return "".join(pieces)


def _format_result(
    *, environment: str, verifier: Mapping[str, Any], response: Mapping[str, Any]
) -> tuple[str, float, dict[str, Any]]:
    text = _format_response_text(response)
    if environment == "citation":
        verifier = _require_exact_keys(
            verifier,
            {"expected_markers", "patterns", "type"},
            name="citation verifier",
        )
        if verifier["type"] != "string_match":
            raise ValueError("citation verifier type differs")
        ...
        if (
            ...
            raise TypeError("citation verifier marker/pattern inventory differs")
        ...
        verifier = _require_exact_keys(
            verifier,
            {"pattern_id", "type", "verify_min_matches", "verify_regex"},
            name="freeform verifier",
        )
        patterns = verifier["verify_regex"]
        minimum = verifier["verify_min_matches"]
        if verifier["type"] != "regex" or type(verifier["pattern_id"]) is not str:
            raise ValueError("freeform verifier identity differs")
        if (
            type(patterns) is not list
            or any(type(item) is not str for item in patterns)
            or type(minimum) is not int
            or not 0 <= minimum <= (1 << 31) - 1
        ):
        ...
        raise ValueError("strict format replay environment is not admitted")
    return text, (1.0 if details["passed"] else 0.0), details


def format_verification_call_expectation(
    *,
    environment: str,
    derived_verifier_request: Mapping[str, Any],
    verifier_response: Mapping[str, Any],
) -> dict[str, Any]:
    """Build one exact format-verifier request/result binding for finalization."""
    profile = _FORMAT_PROFILES.get(environment)
    if profile is None:
        raise ValueError("strict format replay environment is not admitted")
    request_keys = {"responses_create_params", "response", "verifier"}
    if (
        type(derived_verifier_request) is not dict
        or set(derived_verifier_request) != request_keys
        or type(verifier_response) is not dict
    ):
        raise TypeError("format replay request/response must be exact dicts")
    ...
    if set(response) != expected_response_keys:
        raise ValueError("format verifier response keyset differs")
    if any(
        not _exact_json_equal(response[name], request[name])
        for name in ("responses_create_params", "response", "verifier")
    ):
        raise ValueError("format verifier returned request projection differs")
    text, reward, details = _format_result(
        environment=environment,
        verifier=request["verifier"],
        response=request["response"],
    )
    if (
        not _exact_json_equal(response["match_details"], details)
        or type(response["reward"]) is not float
        or (response["reward"] == 0.0 and math.copysign(1.0, response["reward"]) < 0.0)
        or response["reward"] != reward
    ):
        raise ValueError("format verifier result differs from independent result")
    return {
        "profile_id": profile["profile_id"],
        "method": profile["method"],
        "request_sha256": _sha256_bytes(canonical_ascii_json(request)),
        "verifier_sha256": _sha256_bytes(canonical_ascii_json(request["verifier"])),
        "response_text_sha256": _sha256_bytes(canonical_ascii_json(text)),
        "response_sha256": _sha256_bytes(canonical_ascii_json(response)),
        "match_details_sha256": _sha256_bytes(canonical_ascii_json(details)),
        "float_result": reward,
    }


def _validate_format_call(
    document: Any,
    *,
    spec: Mapping[str, Any],
    sequence: int,
    ...
    call = _require_exact_keys(
        ...
        name=f"format verification call {sequence}",
    )
    if (
        call["schema"] != STRICT_GYM_FORMAT_CALL_SCHEMA
        or call["hash_domain"] != STRICT_GYM_CHILD_HASH_DOMAIN
        or call["environment"] != spec["environment"]
    ...
    if not _exact_json_equal(
        ...
        raise ValueError(f"format verification call {sequence} process mismatch")
    call_input = _require_exact_keys(
        call["input"],
    ...
    outcome = _require_exact_keys(
        call["outcome"],
        {"kind", "response_sha256", "match_details_sha256", "float_result"},
...
class StrictGymChildRuntimeSession:
    ...
    def finalize_format_verification_calls(
        self,
        expected_calls: Sequence[Mapping[str, Any]],
        *,
        run_helper: Any,
        ...
        expected_keys = {
            ...
            "response_sha256",
            "match_details_sha256",
            "float_result",
        }
        for sequence, value in enumerate(expected_calls, start=1):
            expected = dict(
                _require_exact_keys(
                    value,
                    expected_keys,
                    name=f"expected format verification call {sequence}",
                )
            )
            if (
                expected["profile_id"] != profile["profile_id"]
                or expected["method"] != profile["method"]
                or any(
                    type(expected[name]) is not str
                    or _SHA256_RE.fullmatch(expected[name]) is None
                    or expected[name] == "0" * 64
                    for name in (
                        "request_sha256",
                        "verifier_sha256",
                        "response_text_sha256",
        ...
        spec, spec_payload = _load_canonical_document(self.spec_path)
        if spec != self.spec or _sha256_bytes(spec_payload) != self.spec_sha256:
            raise RuntimeError("strict Gym child spec changed before finalization")
        child_index_path = self.receipt_root / "index.json"
        child_index, child_index_payload = _load_canonical_document(child_index_path)
        if (
            self._started_index is None
            ...
            or child_index != self._started_index
            or _sha256_bytes(child_index_payload) != self._started_index_sha256
            or child_index.get("environment") != self.environment
            or child_index.get("scope") != "scorer-only"
            or child_index.get("pair_id") != spec["pair_id"]
            or child_index.get("job_id") != spec["job_id"]
        ):
        ...
        processes = getattr(run_helper, "_processes", None)
        if not isinstance(instances, list) or not isinstance(processes, dict):
            raise RuntimeError("format finalization requires the attested RunHelper")
        matches = [
            item
            for item in instances
            if getattr(item, "config_path", None) == target["config_path"]
        ...
        if (
            ...
            raise RuntimeError("format finalization RunHelper identity differs")
        observation = _validate_receipt(
            resource,
            spec=spec,
            target=target,
        ...
        if not _exact_json_equal(observation, child.get("observation")):
            raise ValueError("strict format live observation changed")
        process = resource.get("process")
        if not isinstance(process, Mapping):
            raise ValueError("strict format resource process differs")
        current_ppid, current_start = _process_stat(process["pid"])
        if (
            current_start != process["start_ticks"]
            or current_ppid != process["ppid"]
            or process["boot_id"] != _boot_id()
            or process["hostname"] != socket.gethostname()
        ...
        closed_record = _require_exact_keys(
            ...
            name="strict format verification closed receipt",
        )
        if (
            closed_record["schema"] != STRICT_GYM_FORMAT_CLOSED_SCHEMA
            or closed_record["hash_domain"] != STRICT_GYM_CHILD_HASH_DOMAIN
            or closed_record["environment"] != self.environment
            or closed_record["profile_id"] != profile["profile_id"]
            or closed_record["pair_id"] != spec["pair_id"]
            or closed_record["job_id"] != spec["job_id"]
            or closed_record["spec_sha256"] != self.spec_sha256
            or not _exact_json_equal(
            ...
            or type(closed_record["call_count"]) is not int
            or closed_record["call_count"] != 4
            or not isinstance(closed_record["calls"], list)
            or len(closed_record["calls"]) != 4
        ):
            raise ValueError("strict format verification closed receipt differs")
        records: list[dict[str, Any]] = []
        call_payloads: list[bytes] = []
        for sequence, expected in enumerate(normalized, start=1):
            filename = f"format-verification-call-{sequence:08d}.json"
            document, payload = _load_canonical_document(self.receipt_root / filename)
            _validate_format_call(
                document,
                spec=spec,
                sequence=sequence,
                expected=expected,
        ...
        try:
            ...
            post_shutdown_start = None
        if post_shutdown_start == process["start_ticks"]:
            raise RuntimeError("strict format verifier remained live after shutdown")
        offline = {
            "spec.json": spec_payload,
            "index.json": child_index_payload,
            "resource.json": resource_payload,
            "format-verification-closed.json": closed_payload,
        }
        for sequence, payload in enumerate(call_payloads, start=1):
            offline[f"format-verification-call-{sequence:08d}.json"] = payload
        ...
        terminal = {
            "schema": STRICT_GYM_FORMAT_CALL_INDEX_SCHEMA,
            "hash_domain": STRICT_GYM_CHILD_HASH_DOMAIN,
            "environment": self.environment,
            "profile_id": profile["profile_id"],
            "scope": self.scope,
            ...
            "verification_closed": {
                "path": str(closed_path),
                "sha256": _sha256_bytes(closed_payload),
                "schema": STRICT_GYM_FORMAT_CLOSED_SCHEMA,
            },
            "quiescence": {
                "pid": process["pid"],
                "start_ticks": process["start_ticks"],
                "child_termination_signal": child_signal,
                "wrapper_pid": wrapper_pid,
                "wrapper_returncode": wrapper_returncode,
                "original_process_reaped": True,
            },
        ...
        terminal_path = self.receipt_root / "format-verification-call-index.json"
        root_fd = os.open(self.receipt_root, _DIR_FLAGS)
        try:
            _, digest = _publish_document(root_fd, terminal_path.name, terminal)
        finally:
            os.close(root_fd)
        admitted, admitted_digest = load_finalized_format_verification_call_index(
            terminal_path,
            expected_sha256=digest,
            expected_receipt_root=self.receipt_root,
            expected_pair_id=spec["pair_id"],
        ...
        if admitted != terminal or admitted_digest != digest:
            raise RuntimeError("strict format offline admission differs")
        object.__setattr__(self, "_score_finalized", True)
        return terminal, digest
...
def load_finalized_format_verification_call_index(
    ...
    *,
    ...
    def exact_ref(
        ...
        if not _exact_json_equal(reference, expected):
            raise ValueError(f"{name} differs from the exact artifact")
        return reference
    ...
    if (
        spec["schema"] != STRICT_GYM_CHILD_SPEC_SCHEMA
        or spec["hash_domain"] != STRICT_GYM_CHILD_HASH_DOMAIN
        or spec["environment"] != expected_environment
        or spec["scope"] != "scorer-only"
        or spec["pair_id"] != expected_pair_id
        or spec["job_id"] != expected_job_id
        or not _exact_json_equal(spec["gym"], expected_gym)
    ...
    child_index = _require_exact_keys(
        ...
        name="finalized format child index",
    )
    children = child_index["children"]
    if (
        child_index["schema"] != STRICT_GYM_CHILD_INDEX_SCHEMA
        or child_index["hash_domain"] != STRICT_GYM_CHILD_HASH_DOMAIN
        or child_index["environment"] != expected_environment
        or child_index["scope"] != "scorer-only"
        or child_index["pair_id"] != expected_pair_id
        or child_index["job_id"] != expected_job_id
        or not _exact_json_equal(child_index["gym"], expected_gym)
        ...
        raise ValueError("finalized format child index differs")
    exact_ref(
        child_index["spec"],
        filename="spec.json",
        schema=STRICT_GYM_CHILD_SPEC_SCHEMA,
        name="finalized format child-index spec ref",
    )
    child = _require_exact_keys(
        children[0],
        {"role", "config_path", "receipt", "observation"},
        name="finalized format child",
    )
    exact_ref(
        child["receipt"],
        filename="resource.json",
        schema=STRICT_GYM_CHILD_RECEIPT_SCHEMA,
        name="finalized format resource ref",
    )
    observation = _require_exact_keys(
        child["observation"],
        {
        ...
        name="finalized format observation",
    )
    inodes = observation["listener_socket_inodes"]
    if (
        child["role"] != "resource"
        or child["config_path"] != target["config_path"]
        or type(observation["pid"]) is not int
        ...
        or type(observation["port"]) is not int
        or not 5000 <= observation["port"] <= 5999
        or not isinstance(inodes, list)
        or not inodes
        or any(type(item) is not int or item <= 0 for item in inodes)
        or len(inodes) != len(set(inodes))
    ):
        raise ValueError("finalized format observation differs")
    ...
    resource = _require_exact_keys(
        ...
        name="finalized format resource receipt",
    )
    target_keys = {
        "role",
        "config_path",
    ...
    server = _require_exact_keys(
        ...
        name="finalized format resource server",
    )
    process = _require_exact_keys(
        resource["process"],
        {
    ...
    if (
        resource["schema"] != STRICT_GYM_CHILD_RECEIPT_SCHEMA
        or resource["hash_domain"] != STRICT_GYM_CHILD_HASH_DOMAIN
        or resource["environment"] != expected_environment
        or resource["pair_id"] != expected_pair_id
        or resource["job_id"] != expected_job_id
        or resource["stage"] != "isolated-runner-pre-entrypoint"
        or resource["spec_sha256"] != spec_sha256
        ...
        raise ValueError("finalized format resource receipt differs")

    closed = _require_exact_keys(
        documents["format-verification-closed.json"],
        {
            "schema",
            "hash_domain",
            "environment",
        ...
        name="finalized format closed receipt",
    )
    if (
        closed["schema"] != STRICT_GYM_FORMAT_CLOSED_SCHEMA
        or closed["hash_domain"] != STRICT_GYM_CHILD_HASH_DOMAIN
        or closed["environment"] != expected_environment
        or closed["profile_id"] != expected_profile_id
        or closed["pair_id"] != expected_pair_id
        or closed["job_id"] != expected_job_id
        or closed["spec_sha256"] != spec_sha256
        or not _exact_json_equal(
    ...
    terminal = _require_exact_keys(
        ...
        name="finalized format call index",
    )
    if (
        terminal["schema"] != STRICT_GYM_FORMAT_CALL_INDEX_SCHEMA
        or terminal["hash_domain"] != STRICT_GYM_CHILD_HASH_DOMAIN
        or terminal["environment"] != expected_environment
        or terminal["profile_id"] != expected_profile_id
        or terminal["scope"] != "scorer-only"
        or terminal["pair_id"] != expected_pair_id
        or terminal["job_id"] != expected_job_id
        or type(terminal["call_count"]) is not int
        ...
        raise ValueError("finalized format call-index identity differs")
    exact_ref(
        terminal["spec"],
        filename="spec.json",
        schema=STRICT_GYM_CHILD_SPEC_SCHEMA,
        name="finalized format spec ref",
    )
    exact_ref(
        terminal["child_index"],
        filename="index.json",
        schema=STRICT_GYM_CHILD_INDEX_SCHEMA,
        name="finalized format child-index ref",
    )
    exact_ref(
        terminal["resource_receipt"],
        filename="resource.json",
        schema=STRICT_GYM_CHILD_RECEIPT_SCHEMA,
        name="finalized format resource-receipt ref",
    )
    exact_ref(
        terminal["verification_closed"],
        filename="format-verification-closed.json",
        schema=STRICT_GYM_FORMAT_CLOSED_SCHEMA,
        name="finalized format closed ref",
    )
    digest_names = (
        "request_sha256",
        "verifier_sha256",
    ...
    for sequence in range(1, 5):
        ...
        record = _require_exact_keys(
            terminal["calls"][sequence - 1],
            {"sequence", "method", "input", "outcome", "receipt"},
            name=f"finalized format call record {sequence}",
        )
        call_input = _require_exact_keys(
            record["input"],
            {"request_sha256", "verifier_sha256", "response_text_sha256"},
            name=f"finalized format input {sequence}",
        )
        outcome = _require_exact_keys(
            record["outcome"],
            {"kind", "response_sha256", "match_details_sha256", "float_result"},
            name=f"finalized format outcome {sequence}",
        )
        combined = {**call_input, **outcome}
        if (
            type(record["sequence"]) is not int
            or record["sequence"] != sequence
            or record["method"] != profile["method"]
            ...
            or any(
                ...
                or combined[name] == "0" * 64
                for name in digest_names
            )
        ):
            raise ValueError(f"finalized format call record {sequence} differs")
        expected_closed_ref = {
            "sequence": sequence,
            "path": str(root / filename),
            "sha256": _sha256_bytes(payloads[filename]),
        ...
        receipt_ref = _require_exact_keys(
            record["receipt"],
            {"sequence", "path", "sha256", "schema"},
            name=f"finalized format call ref {sequence}",
        )
        if not _exact_json_equal(receipt_ref, expected_closed_ref):
            raise ValueError(f"finalized format call ref {sequence} differs")
        if not _exact_json_equal(closed["calls"][sequence - 1], expected_closed_ref):
            raise ValueError(f"finalized format closed ref {sequence} differs")
        _validate_format_call(
            documents[filename],
            spec=spec,
            sequence=sequence,
            expected={
                ...
                **call_input,
                ...
                "match_details_sha256": outcome["match_details_sha256"],
                "float_result": outcome["float_result"],
            },
            process=process,
        )
    ...
    quiescence = _require_exact_keys(
        ...
        name="finalized format quiescence",
    )
    child_signal = quiescence["child_termination_signal"]
    if (
        type(quiescence["pid"]) is not int
