...
_FORMAT_CALL_SCHEMA = "nemo-rl-strict-format-verification-call-v1"
_FORMAT_CLOSED_SCHEMA = "nemo-rl-strict-format-verification-closed-v1"
...
_FORMAT_JUDGE_SOURCE = {
    ...
    "sha256": "636d9403775a19c4114b16d176249c6137c10e44a239a8fd74f8b91c18efde1d",
}
_FORMAT_PROFILES = {
    "citation": {
...
def _require_exact_json_tree(value, *, name):
    value_type = type(value)
    if value_type is dict:
        for key, item in value.items():
            if type(key) is not str:
                raise TypeError(f"{name} key is not an exact string")
            _require_exact_json_tree(item, name=name)
...
def _format_response_text(response):
    if type(response) is not dict:
        raise TypeError("format verifier response input must be an object")
...
def _format_expected_result(environment, request):
    profile = _FORMAT_PROFILES[environment]
    if type(request) is not dict or set(request) != {
        "responses_create_params",
        "response",
        "verifier",
    }:
        raise ValueError("format verifier typed request shape differs")
    verifier = request["verifier"]
    if type(verifier) is not dict:
        raise TypeError("format verifier payload must be an object")
    text = _format_response_text(request["response"])
    if environment == "citation":
        if set(verifier) != {"expected_markers", "patterns", "type"}:
            raise ValueError("citation verifier keyset differs")
        if verifier["type"] != profile["verifier_type"]:
        ...
        for pattern in patterns:
            spurious.extend(
                match.group(0)
                for match in _RE_FINDITER(pattern, text)
        ...
        if set(verifier) != {
            ...
            raise ValueError("freeform verifier keyset differs")
        if verifier["type"] != profile["verifier_type"]:
            raise ValueError("freeform verifier type differs")
        if type(verifier["pattern_id"]) is not str:
            raise TypeError("freeform pattern ID must be a string")
        ...
        compiled = [_RE_COMPILE(pattern) for pattern in patterns]
        matching_lines = sum(
            1
        ...
        raise AssertionError("unsupported strict format profile")
    return text, (1.0 if details["passed"] else 0.0), details


def _install_format_score_evidence(
    base_resources_server,
    judge_module,
    *,
    spec,
    spec_sha,
    ...
    if profile is None:
        raise ValueError("format score evidence requires a format profile")
    base_origin = Path(base_resources_server.__file__).resolve(strict=True)
    judge_origin = Path(judge_module.__file__).resolve(strict=True)
    if (
        base_origin != _GYM_ROOT / _FORMAT_BASE_SOURCE["path"]
        or judge_origin != _GYM_ROOT / _FORMAT_JUDGE_SOURCE["path"]
        or _sha_file(base_origin) != _FORMAT_BASE_SOURCE["sha256"]
        or _sha_file(judge_origin) != _FORMAT_JUDGE_SOURCE["sha256"]
        or base_resources_server.judge_failsafe is not judge_module.judge_failsafe
    ):
        raise ValueError("format verifier registration sources differ")
    frozen_factory = judge_module.judge_failsafe
    frozen_factory_code = frozen_factory.__code__
    frozen_factory_globals = frozen_factory.__globals__
    frozen_judge_error = frozen_factory_globals.get("JudgeError")
    frozen_json_response = frozen_factory_globals.get("JSONResponse")
    frozen_jsonable_encoder = frozen_factory_globals.get("jsonable_encoder")
    lock = threading.Lock()
    state = {"registered": False, "in_flight": False, "failed": False, "count": 0}
    call_refs = []

    def validate_factory():
        if (
            judge_module.judge_failsafe is not frozen_factory
            or frozen_factory.__code__ is not frozen_factory_code
            or frozen_factory.__globals__ is not frozen_factory_globals
            or frozen_factory_globals.get("JudgeError") is not frozen_judge_error
            or frozen_factory_globals.get("JSONResponse") is not frozen_json_response
            or frozen_factory_globals.get("jsonable_encoder")
            is not frozen_jsonable_encoder
            or base_resources_server.judge_failsafe is not replacement_factory
            or _sha_file(base_origin) != _FORMAT_BASE_SOURCE["sha256"]
            or _sha_file(judge_origin) != _FORMAT_JUDGE_SOURCE["sha256"]
            or _sha_file(app_path) != spec["targets"][0]["source_sha256"]
        ):
            raise RuntimeError("authenticated format verifier registration changed")

    def replacement_factory(verify_fn):
        with lock:
            if state["registered"]:
                raise RuntimeError("format verifier endpoint registered more than once")
            state["registered"] = True
        instance = getattr(verify_fn, "__self__", None)
        function = getattr(verify_fn, "__func__", None)
        instance_type = type(instance)
        if (
            instance is None
            or function is None
            or instance_type.__name__ != "FormatVerificationResourcesServer"
            or function is not instance_type.verify
            or function.__name__ != "verify"
            or function.__qualname__ != "FormatVerificationResourcesServer.verify"
            or Path(function.__code__.co_filename).resolve(strict=True) != app_path
        ):
            raise RuntimeError("registered format verifier callable differs")
        helpers = {
            "_extract_assistant_text": instance_type._extract_assistant_text,
            "_verify_regex": instance_type._verify_regex,
            "_verify_string_match": instance_type._verify_string_match,
        }
        helper_codes = {name: value.__code__ for name, value in helpers.items()}
        frozen_verify_code = function.__code__
        function_globals = function.__globals__
        request_type = function_globals.get("FormatVerificationVerifyRequest")
        response_type = function.__globals__.get("FormatVerificationVerifyResponse")
        if (
            not isinstance(request_type, type)
            or not isinstance(response_type, type)
            or function.__annotations__.get("body") is not request_type
            or function.__annotations__.get("return") is not response_type
        ):
            raise RuntimeError("format verifier request/response types differ")
        registered_endpoint = frozen_factory(verify_fn)

        def validate_callable():
            validate_factory()
            if (
                function.__code__ is not frozen_verify_code
                or function.__globals__ is not function_globals
                or function_globals.get("FormatVerificationVerifyRequest")
                is not request_type
                or function_globals.get("FormatVerificationVerifyResponse")
                is not response_type
                or function_globals.get("re") is not re
                or re.compile is not _RE_COMPILE
                or re.finditer is not _RE_FINDITER
                or function.__annotations__.get("body") is not request_type
                or function.__annotations__.get("return") is not response_type
                or instance_type.verify is not function
                or any(
                    getattr(instance_type, name) is not value
                    or value.__code__ is not helper_codes[name]
                    for name, value in helpers.items()
                )
            ):
                raise RuntimeError("authenticated format verifier callable changed")

        @functools.wraps(registered_endpoint)
        async def endpoint_with_evidence(*args, **kwargs):
            nonlocal call_refs
            validate_callable()
            with lock:
                if state["failed"] or state["in_flight"] or state["count"] >= 4:
                    raise RuntimeError(
                        "format verifier call entered a closed evidence stream"
                    )
                state["in_flight"] = True
                sequence = state["count"] + 1
            try:
                if args or set(kwargs) != {"body"}:
                    raise TypeError("pinned format verifier invocation differs")
                body = kwargs["body"]
                if type(body) is not request_type:
                    raise TypeError("pinned format verifier request type differs")
                request = body.model_dump()
                if type(request) is not dict:
                    raise TypeError("format verifier typed request dump differs")
                _require_exact_json_tree(request, name="format verifier request")
                request_payload = _canonical(request)
                if len(request_payload) > 64 * 1024 * 1024:
                    raise ValueError("format verifier request exceeds evidence bound")
                returned = await registered_endpoint(*args, **kwargs)
                if type(returned) is not response_type:
                    raise RuntimeError(
                        "format verifier did not return its pinned response type"
                    )
                response = returned.model_dump()
                if type(response) is not dict:
                    raise TypeError("format verifier typed response dump differs")
                _require_exact_json_tree(response, name="format verifier response")
                text, expected_reward, expected_details = _format_expected_result(
                    environment, request
                )
                expected_response = dict(request)
                expected_response.update(
                    {"reward": expected_reward, "match_details": expected_details}
                )
                if _canonical(response) != _canonical(expected_response):
                    raise RuntimeError(
                        "format verifier result differs from independent recomputation"
                    )
                outcome = {
                    "kind": "returned",
                    "response_sha256": hashlib.sha256(_canonical(response)).hexdigest(),
                    "match_details_sha256": hashlib.sha256(
                        _canonical(expected_details)
                    ).hexdigest(),
                    "float_result": expected_reward,
                }
                document = {
                    "schema": _FORMAT_CALL_SCHEMA,
                    "hash_domain": _HASH_DOMAIN,
                ...
                digest = _publish(spec["receipt_root"], filename, document)
                reference = {
                    "sequence": sequence,
                    "path": str(Path(spec["receipt_root"]) / filename),
                    "sha256": digest,
                ...
                with lock:
                    if state["failed"] or not state["in_flight"]:
                        raise RuntimeError("format verifier evidence state changed")
                    state["count"] = sequence
                    call_refs.append(reference)
                    if sequence == 4:
                        closed = {
                            "schema": _FORMAT_CLOSED_SCHEMA,
                            "hash_domain": _HASH_DOMAIN,
                            "environment": environment,
                            "profile_id": profile["profile_id"],
                            "pair_id": spec["pair_id"],
        ...
        validate_callable()
        return endpoint_with_evidence

    base_resources_server.judge_failsafe = replacement_factory
    validate_factory()
...
def _attest():
    ...
    frozen_valid_roles = None
    frozen_scorer_globals = None
    format_base_resources_server = None
    format_judge_module = None
    if target["scorer"] is not None:
        ...
        direct_runner
        and spec["scope"] == "scorer-only"
        and spec["environment"] in _FORMAT_PROFILES
    ):
        import nemo_gym.base_resources_server as imported_base_resources_server
        import nemo_gym.judge as imported_judge_module

        format_base_resources_server = imported_base_resources_server
        format_judge_module = imported_judge_module
    ...
    if format_base_resources_server is not None:
        _install_format_score_evidence(
            format_base_resources_server,
            format_judge_module,
            spec=spec,
            spec_sha=spec_sha,
            process=process,
