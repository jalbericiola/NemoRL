...
"""Fail-closed K=4 scorer-only captured replay.
...
class StrictModelTransportReplaySource(Protocol):
    """Producer-owned exact-once OFF transport source used by this runtime."""

    def consume(self, *, rollout_index: int, generation_seed: int) -> Mapping[str, Any]:
        """Consume one logical source entry exactly once."""

    def record_fresh_verifier_result(self, *, rollout_index: int, verifier_response: Mapping[str, Any]) -> None:
...
IndependentScoreCheck = Callable[[int, Mapping[str, Any], Mapping[str, Any]], None]
FinalizeScoreCallEvidence = Callable[[], Mapping[str, Any]]


@dataclass(frozen=True)
class ReplayDocuments:
    ...
    reasoning_score_call_index: dict[str, Any]


def execute_captured_replay_cohort(
    *,
    ...
    """Execute the admitted K=4 fresh resource-scorer replay.
    ...
    if environment != "reasoning_gym":
        raise StrictCapturedReplayError("only Reasoning-Gym has authenticated scorer-call replay evidence")
    attempt_id = manifest.get("attempt_id")
    if attempt_id not in {"replay-1", "replay-2"}:
        raise StrictCapturedReplayError("replay attempt is not admitted")
    if manifest.get("arm") != "on" or manifest.get("mode") != "fresh_verifier_reward_replay":
    ...
    policy_execution = _mapping(replay_contract.get("policy_execution"), "replay_contract.policy_execution")
    if policy_execution != {
        "backward": False,
        "forward": False,
        "optimizer": False,
    ...
    if manifest.get("wandb") != {
        ...
        raise StrictCapturedReplayError("scorer-only replay requires the exact disabled W&B policy")

    source_capture = _mapping(manifest.get("source_capture"), "source_capture")
    source_step1 = _mapping(source_capture.get("step1_evidence"), "source step1 evidence")
    source_transcript_ref = _mapping(source_step1.get("transcript_bundle"), "source transcript reference")
    source_ledger_ref = _mapping(source_step1.get("main_ledger"), "source ledger reference")
    ...
    if not isinstance(source_entries, list) or len(source_entries) != K4:
    ...
    for rollout_index, source_entry in enumerate(source_entries):
        ...
        if type(generation_seed) is not int:
            raise StrictCapturedReplayError("source generation seed is not an exact int")
        material = _material_document(
            transport_source.consume(rollout_index=rollout_index, generation_seed=generation_seed),
        ...
        if independent_score_check is None:
            raise StrictCapturedReplayError("reasoning_gym replay requires an independent pinned scorer check")
        if independent_score_check is not None:
            independent_score_check(rollout_index, derived_request, verifier_response)
        ...
        if reward == 0.0 and math.copysign(1.0, reward) < 0:
            raise StrictCapturedReplayError("fresh verifier reward must not be negative zero")
        entry_inputs.append(
            {
                ...
                "model_transport_response_body_sha256": material["response_body_sha256"],
            }
        )
    ...
    transport_ref = _mapping(
        _mapping(source_step1.get("model_transport"), "source model transport").get("bundle"),
    ...
    transcript = build_transcript_bundle(
        ...
        verifier_request_derivation=source_transcript_document["verifier_request_derivation"],
    ...
    transcript_output = _mapping(outputs.get("transcript_bundle"), "transcript output")
    transcript_ref = {
    ...
    reasoning_score_call_index_ref = _mapping(finalize_score_call_evidence(), "reasoning score-call terminal reference")
    if set(reasoning_score_call_index_ref) != {"path", "schema", "sha256"}:
        raise StrictCapturedReplayError("reasoning score-call terminal reference keyset differs")
    if reasoning_score_call_index_ref.get("schema") != "nemo-rl-strict-reasoning-score-call-index-v1":
        raise StrictCapturedReplayError("reasoning score-call terminal reference schema differs")
    _digest(
        reasoning_score_call_index_ref.get("sha256"),
        "reasoning score-call terminal SHA-256",
    )
    consumption = _mapping(
        transport_source.finalize(
            ...
            reasoning_score_call_index_ref=reasoning_score_call_index_ref,
        ),
        "transport replay consumption",
    )
    return ReplayDocuments(
        transcript_bundle=transcript,
        replay_ledger=ledger,
        transport_consumption=copy.deepcopy(dict(consumption)),
        reasoning_score_call_index=copy.deepcopy(dict(reasoning_score_call_index_ref)),
    )
...
def post_resource_verify(
    *,
    ...
    payload = canonical_ascii_json(derived_verifier_request)
    ...
    try:
        ...
        if response.status != 200:
            raise StrictCapturedReplayError(f"resource scorer returned HTTP status {response.status}")
        raw = response.read(MAX_HTTP_RESPONSE_BYTES + 1)
        if len(raw) > MAX_HTTP_RESPONSE_BYTES:
            raise StrictCapturedReplayError("resource scorer response exceeds 64 MiB")
        if response.read(1):
            raise StrictCapturedReplayError("resource scorer response grew while reading")
    finally:
        connection.close()
...
def reasoning_gym_score_call_material(
    *,
    ...
    task_name = _mapping(request.get("metadata"), "derived metadata").get("source_dataset")
    if task_name != "knights_knaves":
        raise StrictCapturedReplayError("derived scorer task_name differs from the admitted Reasoning-Gym task")
    extracted_answer = _reasoning_gym_extracted_answer(_reward_facing_text(model_response))
    if response.get("task_name") != task_name or response.get("extracted_answer") != extracted_answer:
        raise StrictCapturedReplayError(
            "fresh Reasoning-Gym response aliases differ from independent request projection"
        )
    ...
    if (
        ...
        or response.get("reward") != score
    ):
        raise StrictCapturedReplayError("fresh Reasoning-Gym score/reward is outside the exact domain")
    return {
...
def _build_replay_ledger_rows(
    *,
    ...
    if not isinstance(source_rows, list) or not isinstance(entries, list):
        raise StrictCapturedReplayError("source ledger/replay transcript rows are malformed")
    ...
    for index, (source_row, entry) in enumerate(zip(source_rows, entries, strict=True)):
        ...
        if source_row.get("prompt_token_ids") != prompt or source_row.get("completion_token_ids") != completion:
            raise StrictCapturedReplayError(f"source ledger row {index} token geometry differs from OFF transport")
...
def _transcript_bindings(
    *,
    ...
    if set(source_snapshot) != {"arm", "ref"} or source_snapshot.get("arm") != "on":
        raise StrictCapturedReplayError("replay source snapshot is not the authenticated ON reference")
...
def _validate_material_against_source(
    *,
    ...
    if canonical_ascii_json(material) != canonical_ascii_json(expected):
        raise StrictCapturedReplayError(f"transport material {rollout_index} differs from authenticated OFF transcript")
...
def _material_document(value: Any, *, rollout_index: int) -> dict[str, Any]:
    ...
    if set(material) != _MATERIAL_KEYS:
        raise StrictCapturedReplayError(f"transport material {rollout_index} keyset differs")
...
def _close_document_reference(
    ...
    *,
    ...
    if reference.get("schema") != expected_schema or document.get("schema") != expected_schema:
...
def _strict_json_object(raw: bytes, *, name: str) -> dict[str, Any]:
    ...
    def reject_duplicate(pairs: Sequence[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in result:
                raise StrictCapturedReplayError(f"{name} contains duplicate key {key!r}")
            result[key] = value
        return result
...
def _expected_float32_rloo_advantages(rewards: Sequence[float]) -> list[float]:
    ...
    for index, reward in enumerate(rounded):
        peers = [value for peer_index, value in enumerate(rounded) if peer_index != index]
        ...
        advantage = _float32(delta / _float32(standard_deviation + 1e-6)) if standard_deviation > 0.0 else delta
...
def _float32(value: Any) -> float:
    if type(value) is not float or not math.isfinite(value):
        raise StrictCapturedReplayError("replay reward must be an exact finite JSON float")
...
def _mapping(value: Any, name: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise StrictCapturedReplayError(f"{name} must be an object")
    return value
...
def _job_id(value: Any) -> str:
    if (
        ...
        raise StrictCapturedReplayError("authenticated job ID must be a canonical positive int63 string")
    return value


def _safe_pair_id(value: Any) -> str:
    if type(value) is not str or _PAIR_ID_RE.fullmatch(value) is None or len(value.encode("ascii")) > 64:
