...
"""Produce fail-closed evidence for strict captured-cohort replay.
...
argv/stdout to :func:`build_captured_replay_submission_receipt`; the independently
authenticated in-job wrapper supplies the successful EXIT observations to
:func:`build_captured_replay_exit_receipt`.  A numeric ID returned by ``sbatch`` is
...
REPLAY_JOB_PRE_RECEIPT_SCHEMA = "nemo-rl-strict-captured-replay-job-pre-receipt-v2"
REPLAY_JOB_EXIT_RECEIPT_SCHEMA = "nemo-rl-strict-captured-replay-job-exit-receipt-v5"
REPLAY_POST_INDEX_SCHEMA = "nemo-rl-strict-captured-replay-evidence-index-v3"
REPLAY_SCHEDULER_QUERY_SCHEMA = "nemo-rl-strict-captured-replay-scheduler-query-v3"
REPLAY_RUNTIME_ATTESTATION_SCHEMA = "nemo-rl-strict-captured-replay-runtime-attestation-v1"

__all__ = [
    "REPLAY_EXIT_ROOT_KEYS",
    "REPLAY_JOB_EXIT_RECEIPT_SCHEMA",
    "REPLAY_JOB_PRE_RECEIPT_SCHEMA",
    "REPLAY_POST_INDEX_ROOT_KEYS",
    "REPLAY_POST_INDEX_SCHEMA",
    "REPLAY_PRE_ROOT_KEYS",
    "REPLAY_RUNTIME_ATTESTATION_KEYS",
    "REPLAY_RUNTIME_ATTESTATION_SCHEMA",
    ...
    "validate_captured_replay_scheduler_query",
...
REPLAY_SUBMISSION_ROOT_KEYS = frozenset(
    {
        "schema",
        "phase",
        "status",
...
REPLAY_PRE_ROOT_KEYS = frozenset(
    {
        "schema",
        "phase",
        "status",
...
REPLAY_EXIT_ROOT_KEYS = frozenset(
    {
        "schema",
        "phase",
        "status",
...
REPLAY_POST_INDEX_ROOT_KEYS = frozenset(
    {
        "schema",
        "hash_domain",
        "pair_id",
...
REPLAY_STATIC_BOUNDARY_KEYS = frozenset(
    {
        "pair",
        "source_capture",
        "replay_contract",
...
REPLAY_RUNTIME_ATTESTATION_KEYS = frozenset(
    {
        "schema",
        "requirements",
        "reasoning_score_call_index",
...
def build_captured_replay_scheduler_query(
    *,
    ...
    manifest = _validated_lifecycle_manifest(replay_execution_manifest, authenticated_source=authenticated_source)
...
def validate_captured_replay_scheduler_query(
    document: Mapping[str, Any],
    *,
    replay_execution_manifest: Mapping[str, Any],
    authenticated_source: AuthenticatedOffSourceCapture,
...
def publish_captured_replay_scheduler_query(
    *,
    output: str | Path,
    document: Mapping[str, Any],
    replay_execution_manifest: Mapping[str, Any],
...
def load_captured_replay_scheduler_query(
    *,
    path: str | Path,
    expected_sha256: str,
    replay_execution_manifest: Mapping[str, Any],
...
def build_captured_replay_submission_receipt(
    *,
    replay_execution_manifest: Mapping[str, Any],
    authenticated_source: AuthenticatedOffSourceCapture,
    ...
    manifest_ref = _artifact_reference(
        ...
        expected_schema=REPLAY_EXECUTION_MANIFEST_SCHEMA,
    )
    ...
    comment = f"nemo-rl-strict-captured-replay-v1:{manifest['attempt_id']}:" f"{nonce}:{manifest_ref['sha256']}"
...
def validate_captured_replay_submission_receipt(
    document: Mapping[str, Any],
    *,
    replay_execution_manifest: Mapping[str, Any],
    authenticated_source: AuthenticatedOffSourceCapture,
    ...
    manifest = _validated_lifecycle_manifest(replay_execution_manifest, authenticated_source=authenticated_source)
    _require_exact_keys(document, REPLAY_SUBMISSION_ROOT_KEYS, name="replay submission receipt")
    expected_envelope = {
        "schema": REPLAY_SUBMISSION_RECEIPT_SCHEMA,
        "phase": "SUBMISSION",
        "status": "held-candidate-not-in-job-authenticated",
    ...
    expected_comment = (
        f"nemo-rl-strict-captured-replay-v1:{manifest['attempt_id']}:" f"{nonce}:{manifest_ref['sha256']}"
    )
    if (
...
def publish_captured_replay_submission_receipt(
    *,
    output: str | Path,
    document: Mapping[str, Any],
    replay_execution_manifest: Mapping[str, Any],
...
def build_captured_replay_pre_receipt(
    *,
    replay_execution_manifest: Mapping[str, Any],
    authenticated_source: AuthenticatedOffSourceCapture,
    ...
    document = {
        "schema": REPLAY_JOB_PRE_RECEIPT_SCHEMA,
        "phase": "PRE",
...
def validate_captured_replay_pre_receipt(
    document: Mapping[str, Any],
    *,
    replay_execution_manifest: Mapping[str, Any],
    submission_receipt: Mapping[str, Any],
    ...
    _require_exact_keys(document, REPLAY_PRE_ROOT_KEYS, name="replay PRE receipt")
    expected_envelope = {
        "schema": REPLAY_JOB_PRE_RECEIPT_SCHEMA,
        "phase": "PRE",
        "status": "authenticated-pre",
    ...
    submission_ref = _artifact_reference(
        ...
        expected_schema=REPLAY_SUBMISSION_RECEIPT_SCHEMA,
    )
...
def publish_captured_replay_pre_receipt(
    *,
    output: str | Path,
    document: Mapping[str, Any],
    replay_execution_manifest: Mapping[str, Any],
...
def load_captured_replay_pre_receipt(
    *,
    path: str | Path,
    expected_sha256: str,
    replay_execution_manifest: Mapping[str, Any],
...
def build_captured_replay_exit_receipt(
    *,
    replay_execution_manifest: Mapping[str, Any],
    authenticated_source: AuthenticatedOffSourceCapture,
    submission_receipt: Mapping[str, Any],
    ...
    pre_ref = _receipt_reference_for_document(
        path=str(pre_path), schema=REPLAY_JOB_PRE_RECEIPT_SCHEMA, document=pre_receipt
    )
    ...
    document = {
        "schema": REPLAY_JOB_EXIT_RECEIPT_SCHEMA,
        "phase": "EXIT",
        "status": "complete",
        "pair_id": manifest["pair_id"],
        "environment": manifest["environment"],
...
def validate_captured_replay_exit_receipt(
    document: Mapping[str, Any],
    *,
    replay_execution_manifest: Mapping[str, Any],
    submission_receipt: Mapping[str, Any],
    ...
    _require_exact_keys(document, REPLAY_EXIT_ROOT_KEYS, name="replay EXIT receipt")
    expected_envelope = {
        "schema": REPLAY_JOB_EXIT_RECEIPT_SCHEMA,
        "phase": "EXIT",
        "status": "complete",
    ...
    pre_ref = _artifact_reference(
        ...
        expected_schema=REPLAY_JOB_PRE_RECEIPT_SCHEMA,
    )
...
def publish_captured_replay_exit_receipt(
    *,
    output: str | Path,
    document: Mapping[str, Any],
    replay_execution_manifest: Mapping[str, Any],
...
def load_captured_replay_exit_receipt(
    *,
    path: str | Path,
    expected_sha256: str,
    replay_execution_manifest: Mapping[str, Any],
...
def build_captured_replay_evidence_index(
    *,
    replay_execution_manifest: Mapping[str, Any],
    authenticated_source: AuthenticatedOffSourceCapture,
    submission_receipt: Mapping[str, Any],
    ...
    document = {
        "schema": REPLAY_POST_INDEX_SCHEMA,
        "hash_domain": HASH_DOMAIN,
        "pair_id": manifest["pair_id"],
        "environment": manifest["environment"],
        "arm": "on",
...
def validate_captured_replay_evidence_index(
    document: Mapping[str, Any],
    *,
    replay_execution_manifest: Mapping[str, Any],
    submission_receipt: Mapping[str, Any],
    ...
    _require_exact_keys(document, REPLAY_POST_INDEX_ROOT_KEYS, name="replay evidence index")
    expected_envelope = {
        "schema": REPLAY_POST_INDEX_SCHEMA,
        "hash_domain": HASH_DOMAIN,
        "pair_id": manifest["pair_id"],
...
def publish_captured_replay_evidence_index(
    *,
    output: str | Path,
    document: Mapping[str, Any],
    replay_execution_manifest: Mapping[str, Any],
...
def load_captured_replay_evidence_index(
    *,
    path: str | Path,
    expected_sha256: str,
    replay_execution_manifest: Mapping[str, Any],
...
def _validate_captured_replay_outputs(
    ...
    *,
    ...
    _require_exact_keys(value, REPLAY_OUTPUT_KEYS, name="replay outputs")
    manifest = replay_execution_manifest
    declared = manifest["artifacts"]["outputs"]
    references: dict[str, dict[str, str]] = {}
    documents: dict[str, dict[str, Any]] = {}
    for name in sorted(REPLAY_OUTPUT_KEYS):
    ...
    score_ref = references["reasoning_score_call_index"]
    score = _load_finalized_reasoning_score_call_index(
        score_ref,
        replay_execution_manifest=manifest,
        authenticated_job_id=authenticated_job_id,
    )
    documents["reasoning_score_call_index"] = score
...
def _load_finalized_reasoning_score_call_index(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError("public finalized reasoning score-call loader is unavailable") from error
    ...
    return _strict_json_object(loaded, name="reasoning score-call terminal index")
...
def _close_replay_output_documents(
    ...
    *,
    ...
    if scorer["terminal_index"] != references["reasoning_score_call_index"] or scorer["status"] != "authenticated":
        raise ValueError("transport consumption scorer terminal differs")

    score = documents["reasoning_score_call_index"]
    _close_score_transcript_join(score, transcript)
...
def _close_score_transcript_join(score_index: Mapping[str, Any], transcript: Mapping[str, Any]) -> None:
    calls = score_index.get("calls")
    if not isinstance(calls, list) or len(calls) != K4_SAMPLES:
        raise ValueError("reasoning score-call index is not exact K=4")
    for index, (call, entry) in enumerate(zip(calls, transcript["entries"], strict=True)):
        ...
        if not isinstance(metadata, Mapping):
            raise TypeError("derived verifier request metadata must be an object")
        scorer_entry = {
        ...
        expected = {
            ...
            "task_name": metadata.get("source_dataset"),
            "input": {
                "answer_sha256": hashlib.sha256(canonical_ascii_json(response.get("extracted_answer"))).hexdigest(),
                "entry_sha256": hashlib.sha256(canonical_ascii_json(scorer_entry)).hexdigest(),
            },
            "float_result": entry["raw_environment_reward"],
        }
        _require_exact_projection(call, expected, name=f"reasoning score-call {index + 1}")
...
def _replay_runtime_attestation(
    ...
    return {
        "schema": REPLAY_RUNTIME_ATTESTATION_SCHEMA,
        "requirements": copy.deepcopy(replay_execution_manifest["runtime_attestation_requirements"]),
        **{name: copy.deepcopy(outputs[name]) for name in REPLAY_OUTPUT_KEYS},
    }
...
def _validated_lifecycle_manifest(
    ...
    *,
    ...
    """Validate a replay manifest against its stable-loaded Pair anchor."""
    document = _strict_json_object(value, name="replay execution manifest")
    # Imported lazily because the manifest utility imports this module's generic
    # canonical document primitives.
    from nemo_rl.utils.strict_captured_replay_manifest import (
        AuthenticatedOffSourceCapture,
        validate_replay_execution_manifest,
    )

    if not isinstance(authenticated_source, AuthenticatedOffSourceCapture):
        raise TypeError("authenticated_source must be an authenticated OFF capability")
    validate_replay_execution_manifest(document, authenticated_source=authenticated_source)
    ...
    if document["environment"] != "reasoning_gym":
        raise ValueError("captured replay lifecycle completion currently requires reasoning_gym")
    return document
...
def _load_replay_scheduler_query_reference(
    ...
    *,
    ...
    query, digest = load_captured_replay_scheduler_query(
        path=query_ref["path"],
...
def _replay_static_boundary(
    ...
    return {name: copy.deepcopy(replay_execution_manifest[name]) for name in REPLAY_STATIC_BOUNDARY_KEYS}
...
def _validate_argv(value: Sequence[str]) -> list[str]:
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes, bytearray)):
        raise TypeError("sbatch.argv must be a sequence of strings")
    if not value:
