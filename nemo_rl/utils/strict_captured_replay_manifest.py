...
REPLAY_SLURM_EXPORT_SCHEMA = "nemo-rl-strict-captured-replay-slurm-export-file-v1"
...
# This is the exact Pair Bootstrap-Trust R1 74-name export set. Replay reuses the closed export
...
def _load_replay_slurm_export(
    *, source: AuthenticatedOffSourceCapture, attempt_id: str
    ...
    if names != list(SLURM_EXPORT_ALLOWED_NAMES):
        raise ValueError("replay Slurm export records must be exact Pair74 order")
    values = dict(records)
...
def build_replay_execution_manifest(
    *,
    ...
    submission_state_root = f"{results_root}/captured_replay/replay_submission_state/" f"{pair_id}/{attempt_id}"
...
def _validate_against_pair(document: Mapping[str, Any], pair: Mapping[str, Any]) -> None:
    ...
    pair_export_names = pair["slurm_export_boundary"]["allowed_names"]
    if not _exact(pair_export_names, list(SLURM_EXPORT_ALLOWED_NAMES)) or not _exact(
        ...
        raise ValueError("replay Slurm export names differ from exact Pair74")
