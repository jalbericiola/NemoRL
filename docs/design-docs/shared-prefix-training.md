...
import-time environment before touching CUDA, MCore must report that its fused
shared-prefix module resolved deterministic FlashAttention backward, and the
resolved Megatron Bridge provider is checked before model construction continues.
...
exact `SHARED_PREFIX_DETERMINISM_ATTESTED ... mcore_backward=true total_controls=9`
log marker with no trailing newline. The launcher must provide a new empty
receipt directory for every job/restart and verify its rank inventory after the
process group exits.
